...
static __always_inline int cell_llc_drain_enable(u32 cell_id, u32 llc)
{
	if (llc >= MAX_LLCS)
		return -EINVAL;

	/*
	 * Look up the map value at the atomic site. Callers often reach this
	 * after cpumask and DSQ kfuncs, and verifier state for an older spilled
	 * cell pointer can merge back as scalar before the atomic access.
	 */
	struct cell *cell = lookup_cell(cell_id);
	if (!cell)
		return -ENOENT;

	__sync_fetch_and_or(&cell->llcs_to_drain, 1LLU << llc);
	return 0;
}

static __always_inline int cell_llc_drain_disable(u32 cell_id, u32 llc)
{
	if (llc >= MAX_LLCS)
		return -EINVAL;

	/* See cell_llc_drain_enable() for why this lookup stays next to the atomic. */
	struct cell *cell = lookup_cell(cell_id);
	if (!cell)
		return -ENOENT;

	__sync_fetch_and_and(&cell->llcs_to_drain, ~(1LLU << llc));
	return 0;
}

static __always_inline int cell_llc_has_cpus(const struct cpumask *cell_mask, u32 llc,
					     bool *has_cpus)
{
	if (llc >= MAX_LLCS)
		return -EINVAL;
	...
	*has_cpus = bpf_cpumask_intersects(cell_mask, llc_mask);
	return 0;
}

static __always_inline int refresh_cell_llc_drain_state(u32 cell_id)
{
	...
	bpf_for(llc, 0, nr_llc)
	{
		...
		dsq_id_t dsq = get_cell_llc_dsq_id(cell_id, llc);
		if (dsq_is_invalid(dsq))
			return -EINVAL;

		bool has_cpus = false;
		if (cell_llc_has_cpus(cell_mask, llc, &has_cpus))
			return -EINVAL;

		if (has_cpus || !scx_bpf_dsq_nr_queued(dsq.raw)) {
			if (cell_llc_drain_disable(cell_id, llc))
				return -EINVAL;
		} else {
			if (cell_llc_drain_enable(cell_id, llc))
				return -EINVAL;
		}
	}
...
static __always_inline int mark_cell_llc_drain_on_enqueue(u32 cell_id, s32 llc)
{
	if (!llc_is_valid(llc) || llc >= nr_llc)
		return 0;
	...
	bool has_cpus = false;
	if (cell_llc_has_cpus(cell_mask, llc_id, &has_cpus))
		return -EINVAL;
	if (has_cpus)
		return 0;

	dsq_id_t dsq = get_cell_llc_dsq_id(cell_id, llc_id);
	if (dsq_is_invalid(dsq))
		return -EINVAL;

	if (scx_bpf_dsq_nr_queued(dsq.raw))
		return cell_llc_drain_enable(cell_id, llc_id);

	return 0;
}

static __always_inline s32 try_drain_cell_llcs(u32 cell_id, s32 local_llc, struct cpu_ctx *cctx)
{
	if (!llc_is_valid(local_llc) || local_llc >= nr_llc) {
		scx_bpf_error("try_drain_cell_llcs: invalid local_llc: %d", local_llc);
		return -EINVAL;
	}
	...
	/*
	 * Drain pass runs independently of local LLC emptiness. Split attempts
	 * across dispatches so normal local work still gets checked frequently.
	 */
	u32 seq = cctx->llc_drain_seq++;
	if (seq & 1)
		return STEAL_WORK_NONE;

	u32 local = (u32)local_llc;
	u32 shift = (seq >> 1) % nr_llc;
	u32 i;
	bpf_for(i, 0, nr_llc)
	{
		u32 offset = (i + shift) % nr_llc;
		u32 candidate_llc = (local + offset) % nr_llc;
		barrier_var(candidate_llc);

		if (candidate_llc >= MAX_LLCS)
			continue;

		if (!(drain_mask & (1LLU << candidate_llc)))
			continue;

		bool has_cpus = false;
		if (cell_llc_has_cpus(cell_mask, candidate_llc, &has_cpus))
			return -EINVAL;

		if (has_cpus) {
			if (cell_llc_drain_disable(cell_id, candidate_llc))
				return -EINVAL;
			continue;
		}
		...
		u64 nr_queued = scx_bpf_dsq_nr_queued(candidate_dsq.raw);
		if (!nr_queued) {
			if (cell_llc_drain_disable(cell_id, candidate_llc))
				return -EINVAL;
			continue;
		...
		/*
		 * If this may be the last queued task, clear before moving and
		 * restore below if work remains. New producers set the bit after
		 * enqueueing into a zero-CPU LLC DSQ.
		 */
		bool cleared = nr_queued <= 1;
		if (cleared && cell_llc_drain_disable(cell_id, candidate_llc))
			return -EINVAL;

		if (!scx_bpf_dsq_move_to_local(candidate_dsq.raw, 0)) {
			if (cleared && scx_bpf_dsq_nr_queued(candidate_dsq.raw) &&
			    cell_llc_drain_enable(cell_id, candidate_llc))
				return -EINVAL;
			continue;
		}
